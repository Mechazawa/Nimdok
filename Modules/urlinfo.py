...
from BotKit import handles, humanize
...
def parse(bot, channel, user, msg):
    for m in msg.split(' '):
        if m[:4] == "http" and '//' in m[5:-len(m)+8]:
            if getdomain(m, True) in ignoreddomains: continue
            try:
                ...
                if "html" in mimetype.lower():
                    ...
                    bot.msg(channel, humanize.Trunicate(ret, 150).replace("\r", "").replace("\n", ""))
                elif "text" in mimetype.lower():
                    txt = urllib2.urlopen(m).read()
                    bot.msg(channel, humanize.Trunicate(txt.replace("\n", " ").replace("\r", "")))
                else:
